...
//    block new_cell;
    ...
    fclose(fp);
        ...
        printf("\nCS-ALLOC_OVERIDE-Error (File_ID:%d Cell_no:%s):Failure to allocate!",file_id,cell_id);
        free(new_cell);
        return -1;
    }
    free(new_cell);
    printf("\nCS-ALLOC_OVERIDE-STATUS (File_ID:%d Cell_no:%s):Allocation completed Successfully",file_id,cell_id);
    return cell_no;
}
